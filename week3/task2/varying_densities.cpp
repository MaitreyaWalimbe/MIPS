...
int main() {
    vector<double> densities = {1.0, 2.0, 3.0, 4.0}; // Different densities

    for (double density : densities) {
        cout << "Simulating for density: " << density << endl;
        L = sqrt(N / density);
        ...
        for (int t = 0; t < total_steps; t++) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                new_thetas[i] = atan2(sy, sx) + (uniform_dist(rng) - 0.5) * noise_level * 2.0 * PI;
            }
        ...
        stringstream fname;
        fname << "C:/Users/Maitreya/mips_simulation/week3/task2/snapshot_density_" << int(density * 10) << ".dat";
